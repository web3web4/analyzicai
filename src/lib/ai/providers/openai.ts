...
export class OpenAIProvider extends BaseAIProvider {
  ...
  constructor(config: AIProviderConfig) {
    ...
    if (!this.model) {
      throw new Error(
        `OpenAI model not configured. Set the ${
          isProduction
            ? "OPENAI_MODEL_FOR_PRODUCTION"
            : "OPENAI_MODEL_FOR_TESTING"
        } environment variable in your environment configuration (e.g. .env, .env.local, or your deployment settings).`,
      );
    }
  }
